...
- **A metadata-only correction does not set `revised`** *(Editor's ruling, 24 August 2026; refines the bullet above)*. `revised` is itself a reader-facing claim — it renders at the page foot, and a reader who sees it reasonably infers that the page's argument or wording moved. So it is set for a change to **what the reader sees on that page**: the body prose, its footnotes, its figures and their captions. It is **not** set for a correction confined to frontmatter that renders somewhere else or not visibly — `description` is the type case, feeding the HTML meta description and the unit landing's TOC blurb but appearing nowhere on the section page itself. Setting `revised` for such a pass tells the reader something *less* true than leaving it alone: it announces a revision that, on the page in front of them, did not happen. Git history remains the full record either way. Where one pass touches both, `revised` follows the body change. **Occasion:** the 24 August 2026 sweep that struck Locus numbering from ten Cycle-2 theology stubs' `description` fields (FWI-057) — eight of them live, none of them visibly changed, and eight "Revised" lines would have overstated every one.
